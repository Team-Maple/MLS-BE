...
- `service.version=${SERVICE_VERSION:-unknown}`; deploy runner가 image의 검증된 OCI source
  revision label을 candidate에 주입한다.
...
운영 애플리케이션 배포 진입점은 `.github/workflows/deploy-oci.yml` 하나다. Main-only
`production-build` Environment의 build job이 image를 publish한 뒤, 별도 runner의 deploy
job이 GitHub `production` Environment owner 승인을 한 번 받고 host를 변경한다.
`production-build`에는 reviewer를 두지 않고 deployment record도 만들지 않는다.
...
### 한 번만 수행하는 runner와 Compose override 전환
