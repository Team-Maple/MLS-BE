...
GitHub CI key의 `authorized_keys` 항목은 `command="/opt/mapleland/update-api.sh"`, `no-port-forwarding`, `no-agent-forwarding`, `no-X11-forwarding`, `no-pty`를 유지한다. 따라서 SSH client가 보낸 command는 직접 실행되지 않고 강제 entrypoint의 stdin으로만 전달된다. Non-root entrypoint는 newline 없는 단일 line 전체를 정규식으로 검증하며 `preflight <3 checksums>` 또는 `deploy <3 checksums> <repository@sha256:digest>`만 허용한다. Shell `eval`, tag image, 임의 command와 추가 argument는 거부한다. Allowlist를 통과한 경우에만 `/usr/bin/sudo -n`으로 고정된 preflight/update script를 호출하고, root로 재진입한 update script가 실제 배포를 수행한다.
